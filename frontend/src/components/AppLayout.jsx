...
export default function AppLayout({ user, onLogout, children }) {
  ...
  return (
    <div className="flex min-h-screen bg-[#F7F7F5]">
      <aside
        data-testid="sidebar"
        className="fixed inset-y-0 left-0 w-[220px] bg-[#1F2420] text-[#E8E8E4] flex flex-col z-30"
      >
        <div className="flex items-center gap-2.5 px-4 h-[52px] border-b border-white/10">
          <div className="w-7 h-7 rounded-[4px] bg-[#D8621B] flex items-center justify-center shrink-0">
            <Construction size={16} className="text-white" />
          </div>
          <div className="leading-tight">
            <div className="text-[13px] font-semibold tracking-tight">Sucoot Scaform</div>
            <div className="text-[10px] text-white/45">Sistem Penyewaan &amp; Penjualan</div>
          </div>
        </div>
        <nav className="flex-1 py-3 px-2 space-y-0.5">
          {NAV.map(({ to, label, icon: Icon }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-2.5 px-2.5 py-[7px] rounded-[4px] text-[13px] transition-colors ${
                  isActive
                    ? "bg-[#D8621B] text-white font-medium"
      ...
      <div className="flex-1 ml-[220px] flex flex-col min-w-0">
        <header
          data-testid="topbar"
          className="sticky top-0 z-20 h-[52px] bg-white border-b border-[#E4E4E0] flex items-center gap-4 px-5"
        >
          <div className="flex items-center gap-1.5 text-[12px] text-[#1F2420]/55 min-w-0">
            {crumb.split(" / ").map((part, i, arr) => (
              <span key={i} className="flex items-center gap-1.5 whitespace-nowrap">
                <span className={i === arr.length - 1 ? "text-[#1F2420] font-medium" : ""}>{part}</span>
                {i < arr.length - 1 && <ChevronRight size={12} className="text-[#1F2420]/30" />}
              </span>
            ))}
          </div>
          ...
          <div className="relative w-[260px] hidden md:block">
            ...
            <input
              ...
              className="w-full h-[30px] pl-8 pr-3 text-[12px] bg-[#F7F7F5] border border-[#E4E4E0] rounded-[4px] outline-none focus:border-[#D8621B]/60"
            />
          </div>
          <div className="flex items-center gap-2.5 pl-3 border-l border-[#E4E4E0]">
            <div className="w-7 h-7 rounded-[4px] bg-[#2E3B4E] text-white text-[11px] font-semibold flex items-center justify-center">
              {user.nama.slice(0, 2).toUpperCase()}
            </div>
            <div className="leading-tight hidden sm:block">
              <div className="text-[12px] font-medium">{user.nama}</div>
              <div className="text-[10px] text-[#1F2420]/50">{user.role}</div>
            </div>
            <button
              data-testid="logout-btn"
              onClick={() => { onLogout(); navigate("/login"); }}
              title="Keluar"
              className="p-1.5 rounded-[4px] text-[#1F2420]/45 hover:text-[#B3452F] hover:bg-[#B3452F]/5 transition-colors"
            >
              <LogOut size={15} />
            </button>
          </div>
